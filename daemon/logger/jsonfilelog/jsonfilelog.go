...
func New(ctx logger.Context) (logger.Logger, error) {
	if err := validateLogOpts(ctx.Config); err != nil {
		return nil, err
	}
...
func (l *JSONFileLogger) Log(msg *logger.Message) error {
	...
	if err := (&jsonlog.JSONLogBytes{Log: append(msg.Line, '\n'), Stream: msg.Source, Created: timestamp}).MarshalJSONBuf(l.buf); err != nil {
		return err
	}
	l.buf.WriteByte('\n')
	_, err = writeLog(l)
