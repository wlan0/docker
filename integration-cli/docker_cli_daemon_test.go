...
func TestDaemonLoggingDriverDefault(t *testing.T) {
	...
	logDone("daemon - 'default' logging driver")
}
...
func TestDaemonLoggingDriverNoneOverride(t *testing.T) {
	...
	out, err := d.Cmd("run", "-d", "--log-driver=default", "busybox", "echo", "testline")
	if err != nil {
		t.Fatal(out, err)
	}
	id := strings.TrimSpace(out)
...
func TestDaemonLoggingDriverNoneLogsError(t *testing.T) {
	...
	if !strings.Contains(out, `\"logs\" command is supported only for \"default\" logging driver`) {
		t.Fatalf("There should be error about non-default driver, got %s", out)
	}
	logDone("daemon - logs not available for non-default drivers")
}
